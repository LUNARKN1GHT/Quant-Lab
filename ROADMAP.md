...
- [ ] **统一模型接口**：重构 `ml_alpha.py`，支持传入任意 sklearn 兼容模型（先做这个）
...
- [ ] 实现 `quant/regime/detector.py`：基于规则的市场状态分类
  - **趋势型信号**：沪深300当前价格 vs 120日/250日均线（金叉/死叉）
  - **宽度型信号**：沪深300中上涨股票占比（市场宽度）、新高/新低比例
  - **量能信号**：成交额 20日均值变化率，缩量下跌 vs 放量下跌
  - **波动率信号**：VIX类似物（如沪深300隐含波动率，或历史波动率骤升）
- [ ] 输出三态：`BULL（趋势上行）/ RANGE（震荡）/ BEAR（趋势下行）`
- [ ] 回测不同市场状态下各策略的表现差异
...
- [ ] 接入基金净值数据（`ak.fund_open_fund_info_em`，支持公募基金）
- [ ] 实现基金评估指标：Sharpe、最大回撤、超额收益（相对沪深300）、胜率
- [ ] 基金风格分析：将基金收益回归到风格因子（价值/成长/小盘/大盘），理解实际风险暴露
- [ ] 基金相关性矩阵：识别持仓中重叠度高的基金（避免伪分散）
- [ ] 持仓组合归因：当前基金组合的行业/风格分布，对标自己的资产配置目标

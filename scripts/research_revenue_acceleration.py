...
def load_data(con):
    fundamental = con.execute("""
        SELECT symbol, report_date, disclose_date, revenue_yoy
...
def main():
    ...
    print("计算财务加速度因子...")
    factor_matrix = revenue_acceleration_pit(fundamental, price_dates, symbols)
    ...
    (pivot["G5"] - pivot["G1"]).cumsum().plot(ax=axes[2], color="#1f77b4")
    axes[2].set_title("多空累计收益（G5 - G1）")
    axes[2].axhline(0, color="black", linewidth=0.8)

    plt.tight_layout()
    out = OUT_DIR / "factor_layered_revenue_acceleration.png"
